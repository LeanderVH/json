...
#print('------3C---------')
#print(servers_struc["rack"][0]["server"]["services"][0])


print('----server 1 ip adress----')
print(servers_struc["rack"][0]["server"]["ip-address"])

print('-----tweede poort laatste server-----')
print(servers_struc["rack"][2]["server"]["services"][1]["port"])

print('----naam en ip adress alle servers----')
#eerst voor 1 server de output maken
#print(servers_struc["rack"][0]["server"]["server_name"] + " " + servers_struc["rack"][0]["server"]["ip-address"])
#dan voor alles een for loop maken
for s in servers_struc["rack"]:
    print(s ["server"]["server_name"] + " " + s["server"]["ip-address"])

print('----alle poorten van server 3-----')
#print(servers_struc["rack"][2]["server"]["services"][0]["port"])
for a in servers_struc["rack"][2]["server"]["services"]:
    print(a ["port"])